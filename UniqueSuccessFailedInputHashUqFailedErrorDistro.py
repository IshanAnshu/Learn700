                ...
                if (logJson['AuthType'] == 'FID' and logJson['Status'] == 'failed') or (
                    ...
                    if logJson['ErrorCode'] == "K-100":
                        ...
                        try:
                            ...
                            if K100Err == "930":
                                dictInputHash[logJson['InputHash']].append("K-100 930")
                            if K100Err == "935":
                                dictInputHash[logJson['InputHash']].append("K-100 935")
...
def printResult(count):
    ...
    for values in dictInputHash.values():
        ...
        if values[1]==0:
            ...
            if len(values) > 2:
                if values[2] in dictUniqueFailedCountErrorList.keys():
                    dictUniqueFailedCountErrorList[values[2]]+=1
                else:
                    dictUniqueFailedCountErrorList[values[2]] = 1
